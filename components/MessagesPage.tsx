...
export const MessagesPage: React.FC<MessagesPageProps> = ({ onMovieSelect }) => {
    ...
    return (
        <div className="min-h-screen bg-[#0f1014] px-0 pb-20 pt-0 md:px-10 md:pb-10 md:pt-20">
            <div className="mx-auto w-full max-w-[1500px]">
                <div className="mb-6 px-5 pt-6 md:px-8 md:pt-0">
                    <div className="flex items-start justify-between gap-6">
                        <div>
                            <div className="text-[11px] font-bold uppercase tracking-[0.28em] text-zinc-500">
                                Mutual Messages
                            </div>
                            <h1 className="mt-3 text-5xl font-extralight tracking-tight text-white md:text-6xl">
                                Messages
                            </h1>
                            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-zinc-400">
                                Send direct messages and share titles with people who follow you back. The follow relationship is enforced server-side, not just in the UI.
                            </p>
                        </div>

                        <div className="hidden rounded-full border border-white/10 bg-white/5 px-4 py-2 text-[11px] font-bold uppercase tracking-[0.18em] text-zinc-300 md:inline-flex">
                            {profile?.username || 'Inbox'}
                        </div>
                    </div>
                </div>

                <div className="grid min-h-[72vh] grid-cols-1 gap-4 md:grid-cols-[360px_minmax(0,1fr)]">
                    <aside className={`${selectedConversationId ? 'hidden md:flex' : 'flex'} min-h-[540px] flex-col overflow-hidden rounded-[32px] border border-white/8 bg-[#111216] shadow-[0_18px_60px_rgba(0,0,0,0.38)]`}>
                            ...
                            <div className="rounded-[24px] border border-white/8 bg-black/20 p-3">
                                <div className="flex items-center gap-3 rounded-[18px] border border-white/10 bg-white/5 px-4 py-3">
                                    ...
                                    <input
                                        ...
                                        placeholder="Search mutuals or conversations..."
                        ...
                        <div className="flex-1 space-y-5 overflow-y-auto p-5 custom-scrollbar">
                            <section>
                                ...
                                <div className="space-y-2">
                                    {loading && (
                                        <div className="rounded-[22px] border border-white/8 bg-white/5 px-4 py-4 text-sm text-zinc-400">
                                            Loading inbox...
                                        </div>
                                    )}

                                    {!loading && filteredConversations.length === 0 && (
                                        <div className="rounded-[22px] border border-dashed border-white/10 bg-white/[0.03] px-4 py-5 text-sm leading-relaxed text-zinc-500">
                                        ...
                                        <button
                                            ...
                                            className={`w-full rounded-[22px] border px-4 py-4 text-left transition-all ${
                                                selectedConversationId === conversation.id
                                                    ? 'border-white/18 bg-[linear-gradient(180deg,rgba(255,255,255,0.12),rgba(255,255,255,0.05))]'
                                ...
                                <div className="space-y-2">
                                    ...
                                    {suggestedProfiles.map((candidate) => (
                                        <button
                                            ...
                                            className="flex w-full items-center justify-between gap-3 rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4 text-left transition-all hover:border-white/14 hover:bg-white/[0.05] disabled:cursor-wait disabled:opacity-60"
                                        >
                    ...
                    <section className={`${selectedConversationId ? 'flex' : 'hidden md:flex'} min-h-[540px] flex-col overflow-hidden rounded-[32px] border border-white/8 bg-[#111216] shadow-[0_18px_60px_rgba(0,0,0,0.38)]`}>
                        {selectedConversation ? (
                            <>
                                ...
                                <div className="flex-1 space-y-4 overflow-y-auto px-5 py-5 custom-scrollbar">
                                    ...
                                    {!threadLoading && messages.length === 0 && (
                                        <div className="flex h-full min-h-[280px] flex-col items-center justify-center rounded-[28px] border border-dashed border-white/10 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.06),transparent_70%)] px-6 py-10 text-center">
                                ...
                                <div className="border-t border-white/6 p-5">
                                    ...
                                    <div className="flex items-end gap-3 rounded-[26px] border border-white/10 bg-black/20 p-3">
                                        <textarea
                                            value={composer}
                            ...
                            <div className="flex h-full min-h-[540px] flex-col items-center justify-center px-8 text-center">
